...
sourcex "/usr/libexec/rc/rc/sh/functions.sh"
sourcex "/usr/libexec/rc/rc/sh/rc-functions.sh"
case $RC_SYS in
	PREFIX|SYSTEMD-NSPAWN) ;;
	*) yesno "$RC_USER_SERVICES" || sourcex -e "/usr/libexec/rc/rc/sh/rc-cgroup.sh";;
esac
...
sourcex -e "/etc/openrc/rc.conf"
if [ -d "/etc/openrc/rc.conf.d" ]; then
	for _f in "/etc/openrc"/rc.conf.d/*.conf; do
		sourcex -e "$_f"
	done
fi
...
sourcex "/usr/libexec/rc/rc/sh/runit.sh"
sourcex "/usr/libexec/rc/rc/sh/s6.sh"
sourcex "/usr/libexec/rc/rc/sh/start-stop-daemon.sh"
sourcex "/usr/libexec/rc/rc/sh/supervise-daemon.sh"
